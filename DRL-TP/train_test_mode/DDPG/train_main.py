...
#=======================================#
#      Train DDPG algorithm offline     #
#=======================================#
class RLDQN:
    def __init__(self):
        # --- train/test params --- #
        print(" ..... LOAD DDPG MAIN ......")
        self.batch_size = 64
        ...
        self.writer = SummaryWriter(self.path_obj.get_save_path(type="LOG"))
        self.agent = Agent(self.env.k_paths, self.env.action_dim, self.batch_size, self.hidden_size, self.gamma,
                           self.lr, self.writer)
    ...
    def breakpoint_train(self):
        ...
        if data:
            self.agent.policy_actor.load_state_dict(torch.load(data[1]))
            return data[0]
        else:
            return 0
    ...
    def train_model(self):
        ...
        for t in range(start_t * self.frequency_ddpg_network + 1, self.total_train_episodes):
            episode_reward = 0.
            episode_total_step = 0
            # save_reward = 0.
            state = self.env.reset()
            while True:
                episode_total_step += 1
            ...
            if (t + 1) % self.frequency_ddpg_network == 0:
                torch.save(self.agent.policy_actor.state_dict(),
                           self.path_obj.get_save_path(type="DDPG") + "actor.pth")
                torch.save(self.agent.policy_critic.state_dict(),
                           self.path_obj.get_save_path(type="DDPG") + "critic.pth")
                # torch.save(self.agent.policy_actor.state_dict(), os.path.abspath(os.path.dirname(os.path.dirname())))
                print("\033[33;1m POLICY NETWORK[ACTOR, CRITIC] HAS BEEN SAVED in [%s]\033[0m" % (self.wacth_time()))
        ...
        try:
            ...
            with open(os.path.abspath(os.path.dirname(os.path.dirname(self.path_obj.get_save_path(type="REWARD")))) + "/explain.txt",
                      "w") as f:
                title = "***The influence of GRU K=8 DDPG weight on the experiment was measured***\n"
