    ...
    def train_model(self, type="Train"):
        ...
        for epoch in range(self.episodes):
            ...
            for i, data in enumerate(train_loader):
                input, target = data  # 7, k, 196 | 1, k, 196
                input = torch.tensor(input, dtype=torch.float).squeeze(0).permute(1, 2, 0).cuda()  # k, 196, 7
                target = torch.tensor(target, dtype=torch.float).squeeze(0).cuda()
                out, out_hidden = self.gru(input, hidden)
                out = out.permute(2, 0, 1).squeeze(0)   # k, 196
                self.optimizer.zero_grad()
                loss = self.criterion(out, target.squeeze(0))
                epoch_loss += loss.item()
                loss.backward()
                self.optimizer.step()
                hidden = out_hidden.detach()            # 释放图
        ...
        with open(self.path_obj.get_save_path(type="GRU_LOSS").split(".")[0] + ".txt", "w") as f:
