...
class RLDQN:
    def __init__(self):
        ...
        # self.frequency_dqn_network = 1000
    ...
    def breakpoint_train(self):
        ...
        if data:
            self.agent.policy_network.load_state_dict(torch.load(data[1]))
            return data[0]
        else:
            return 0
    ...
    def train_model(self):
        ...
        decay = start_t * self.frequency_dqn_network + 1
        for t in range(start_t * self.frequency_dqn_network + 1, self.total_train_episodes):
            episode_reward = 0.
            # save_reward = 0.
            state = self.env.reset()
            while True:
                action = self.agent.action(decay, state)
                # print(action.shape)
                # assert 0
                ...
                if self.use_rnn_network:
                    ...
                    if seq_previous_traffic is not None:
                        ...
                        # command predict state step
                        action = self.agent.action(decay, predict_state)
                        reward, done, _ = self.env.predict_step(action)
                        # predict_state as next state
                        self.agent.memory.store_experience(state, action, reward, predict_state, done)
                        decay += 1
                state = next_state
                # update policy network #
                if self.agent.memory.can_sample(batch_size=self.batch_size):
                    self.agent.update()
                # exit loop
                if done:
                    break
            ...
            if (t + 1) % self.frequency_step == 0:
                self.agent.target_network.load_state_dict(self.agent.policy_network.state_dict())
            # save dqn policy network
            if (t + 1) % self.frequency_dqn_network == 0:
                torch.save(self.agent.policy_network.state_dict(), self.path_obj.get_save_path(type="DQN"))
                print("\033[33;1m POLICY NETWORK [%s] HAS BEEN SAVED in [%s]\033[0m" % (
                self.path_obj.dqn_idx, self.wacth_time()))
        ...
        try:
            ...
            with open(os.path.abspath(os.path.dirname(os.path.dirname(self.path_obj.get_save_path(type="REWARD")))) + "/explain.txt",
                      "w") as f:
                title = "***The influence of GRU K=8 DDQN weight on the experiment was measured***\n"
