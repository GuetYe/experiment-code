...
class Env:
    def __init__(self, metric_type="Train"):
        ...
        # --- test traffic's factor and function's factor -- #
        self.weight_factors = [0.6, 0.3, 0.1, 0.0, 0.0, 0.0]
    ...
    def min_max_normalization(self, traffic):
        """
            Min-Max technology to normal matrix.
        :param no_norm_matrix:
        :return:
        """
        max_item = np.nanmax(traffic)
        min_item = np.nanmin(traffic)
        for row in range(traffic.shape[0]):
            traffic[row, :] = 0 + ((traffic[row, :] - min_item)*(1 - 0)) / (max_item - min_item)
        return traffic

    def normal_traffic_matrics(self, traffic):
        """
             Mean-std technology to normal matrix.
        :return:
        """
        if traffic is None:
            return
    ...
    def generate_all_metric_infos(self, one_state_metric_infos, metric_idx):
        ...
        for src in range(nomal_metri_infos.shape[2]):       # 14
            for dst in range(nomal_metri_infos.shape[3]):   # 13
                ...
                for k in range(nomal_metri_infos.shape[0]):
                    one_path_price = 0.
                    # check_list = []
                    for m in range(nomal_metri_infos.shape[1]):  # 6 ["free_bw", "delay", "packet_loss", "used_bw", "packet_drop", "packet_errors"]
