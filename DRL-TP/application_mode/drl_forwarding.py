...
from thread import start_new_thread, get_ident, exit_thread

CONF = cfg.CONF


class DRLForwarding(app_manager.RyuApp):
    """
    ...
    def __init__(self, *args, **kwargs):
        super(DRLForwarding, self).__init__(*args, **kwargs)
        self.name = "drl_algorithm"
        # load module # 
        ...
        self.execute_drl_flag = False  # the flag of drl
        ...
        self.candidate_path = None      # save all alternative paths
        self.drl_path = {}              # save drl path by drl algo

        # evaluating indicator #
        self.drl_link_path = defaultdict(dict)
        self.drl_link_throughput= defaultdict(dict)
        self.drl_link_delay = defaultdict(dict)
        self.drl_link_loss = defaultdict(dict)
        self.drlpr = DRL()

        self.json_obj = SaveInfosToJson()
        self.path_obj = self.json_obj.make_path
        # self.prepare_iperf_drl_flag = True

        # create hub spawn #
        self.metric_spawn = hub.spawn(self._detect_all_metric)
    ...
    def _detect_all_metric(self):
        ...
        while True:
            ...
            if self.candidate_path is None:
                self.candidate_path = self.path_obj.get_topo_data()
            # intelligent routing algorithm #
            if self.candidate_path and self.execute_drl_flag:
                traffic_matrix = defaultdict(dict)
                for src in self.candidate_path.keys():
                    for dst in self.candidate_path[src].keys():
                        if src == dst: continue
                        # obtain the link information matrix[errors, drops, loss, delay ...] of the current path.
                        traffic_matrix[src][dst] = self.management_module.get_traffic_matrix(self.candidate_path[src][dst])
                # check the traffic's matrix is format #
                if self.check_metric_is_format(traffic_matrix) == CF.NODES * (CF.NODES - 1):
                    self.drl_path = self.drlpr.get_optimal_forwarding_path(self.all_metric_infos)             # drl-tp
            hub.sleep(CF.MONITOR_PERIOD + 0.05)
    ...
    def save_all_link_indicators(self):
        ...
        :return:
        """
        self.json_obj.save_all_path_infos(self.drl_link_path, type="DRL_PATH")
        self.json_obj.save_all_metrics_infos(self.drl_link_throughput, type="DRL_THROUGHTPUT_PATH")
        self.json_obj.save_all_metrics_infos(self.drl_link_delay, type="DRL_DELAY_PATH")
        self.json_obj.save_all_metrics_infos(self.drl_link_loss, type="DRL_LOSS_PATH")
        self.drl_link_path.clear()
        self.drl_link_throughput.clear()
        self.drl_link_delay.clear()
        self.drl_link_loss.clear()
        print("\033[35;1m drl [%s] done !!!\033[0m" % get_ident())
        # exit_thread()
        ...
        if result:
            ...
            if dst_sw:
                ...
                if tos != 0:
                    ...
                    self.drl_link_path[str(src_sw)][str(dst_sw)]  = path
                    self.drl_link_delay[str(src_sw)][str(dst_sw)] = delay
                    self.drl_link_loss[str(src_sw)][str(dst_sw)] = loss
                    self.drl_link_throughput[str(src_sw)][str(dst_sw)] = throughput
