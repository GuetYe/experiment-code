...
class DIJSKRAForwarding(app_manager.RyuApp):
    """
    ...
    def __init__(self, *args, **kwargs):
        super(DIJSKRAForwarding, self).__init__(*args, **kwargs)
        ...
        self.dijkstra_link_path = defaultdict(dict)
        self.dijkstra_link_delay = defaultdict(dict)
        self.dijkstra_link_loss = defaultdict(dict)
        self.dijkstra_link_throughput = defaultdict(dict)
    ...
    def save_all_link_indicators(self):
        ...
        :return:
        """
        if self.check_metric_is_format(self.dijkstra_link_path) == CF.NODES * (CF.NODES - 1):
            self.json_obj.save_all_path_infos(self.dijkstra_link_path, type="DIJSKRA_PATH")
            self.json_obj.save_all_metrics_infos(self.dijkstra_link_throughput, type="DIJSKRA_THROUGHTPUT_PATH")
            self.json_obj.save_all_metrics_infos(self.dijkstra_link_delay, type="DIJSKRA_DELAY_PATH")
            self.json_obj.save_all_metrics_infos(self.dijkstra_link_loss, type="DIJSKRA_LOSS_PATH")
            self.dijkstra_link_path.clear()
            self.dijkstra_link_throughput.clear()
            self.dijkstra_link_delay.clear()
            self.dijkstra_link_loss.clear()
            print("\033[35;1m dijkstra [%s] done !!!\033[0m" % get_ident())
        ...
        if result:
            ...
            if dst_sw:
                ...
                # dijkstra algo #
                    ...
                    path = self.topology_module.get_dijkstra_path(src_sw, dst_sw)
                ...
                if tos != 0:
                    ...
                    self.dijkstra_link_path[str(src_sw)][str(dst_sw)] = path
                    self.dijkstra_link_delay[str(src_sw)][str(dst_sw)] = delay
                    self.dijkstra_link_loss[str(src_sw)][str(dst_sw)] = loss
                    self.dijkstra_link_throughput[str(src_sw)][str(dst_sw)] = throughput
